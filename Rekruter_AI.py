...
# --- NOWA KONFIGURACJA (PROJEKT AI-REKRUTER) ---
BUCKET_NAME = "rekrutacja-pliki-2026"
GCP_PROJECT_ID = "ai-rekruter"
GCP_GEMINI_LOCATION = "europe-central2"
GCP_SEARCH_LOCATION = "global"  # Zmieniono na global zgodnie z linkiem
...
def setup_gcp_clients():
    ...
    # Vertex AI wymaga pliku lub zmiennej env. Tworzymy bezpieczny plik tymczasowy.
    ...
    try:
        ...
        # 3. Discovery Engine (Search) - Obsługa lokalizacji Global
        ...
        if os.path.exists(temp_file.name):
            os.remove(temp_file.name)
        raise Exception(f"Błąd inicjalizacji GCP: {e}")


# Wywołanie inicjalizacji przy starcie modułu
try:
...
# --- INTERFEJS KANDYDATA (Funkcja wywoływana przez app.py) ---
    ...
    if not st.session_state.cv_uploaded_id:
        ...
        if uploaded:
            with st.spinner("Analiza..."):
                ...
                errors = bigquery_client.dataset(BIGQUERY_DATASET_ID).table(BIGQUERY_TABLE_ID).insert_rows_json([row])

                if not errors:
                    st.session_state.cv_uploaded_id = cid
                    msg = f"Cześć {analysis['candidate_name']}! Opowiedz o swoim doświadczeniu."
                    st.session_state.messages = [{"role": "assistant", "content": msg}]
                    st.rerun()
                else:
                    st.error(f"Błąd BigQuery: {errors}")
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    ...
                    if ended:
                        ...
                        bigquery_client.dataset(BIGQUERY_DATASET_ID).table(BIGQUERY_TABLE_ID).insert_rows_json([row])